...
class MeasurementsManager:
    def __init__(self, chunk_size: int, k_reactor=None, stdata_filename: Optional[Path] = None):
        ...
        if stdata_filename is not None:
            ...
            current_final_file: Path = stdata_filename
        ...
        self._is_writing: Any = Value('b', False)
    ...
    @staticmethod
    def _writer_loop(output_file: Path, write_queue: Queue, is_writing: Any):
        try:
    ...
    def add_measurement(self, name: str, samples: Optional[SamplesList] = None, timeout: float = WRITE_TIMEOUT):
        ...
        if self._writer_process is None:
            ...
            assert self._writer_process is not None  # Explicit assertion for type checker
        ...
        if len(self.measurements) > self._chunk_size:
            ...
            # This prevents starting a new measurement while the previous one is still being written to disk.
            # This is crucial to avoid "Timer too close" errors in Klipper.
    ...
    def save_stdata(self, timeout: int = WRITE_TIMEOUT):
        ...
        assert self._writer_process is not None
        eventtime = self._k_reactor.monotonic()
        endtime = eventtime + timeout
        complete = False
        while eventtime < endtime:
        ...
        for logname in klipper_csvs:
            try:
                ...
                with open(logname) as f:
                    header: Optional[str] = None  # Explicitly type header as Optional[str]
                    for line in f:
                    ...
                    if not header:
                        ConsoleOutput.print(
                            ...
                            f"Expected '#time,accel_x,accel_y,accel_z', but got '(no header found)'. "
                    ...
                    samples = [cast(Sample, tuple(float(x) for x in row)) for row in data]
                    if os.environ.get('SHAKETUNE_IN_CLI') != '1':
                        # When running as a Klipper plugin, use the standard add_measurement method.
                        self.add_measurement(name=logname.stem, samples=samples)
                    else:
                        # In CLI mode, manually append samples.
                        # The add_measurement method requires a temp file for writing,
                        # which is not available or needed in CLI mode.
                        # CLI mode is typically for single-shot runs, so this is acceptable.
...
class Accelerometer:
    def __init__(self, klipper_accelerometer, k_reactor):
        ...
        self._measurements_manager: MeasurementsManager = Any
    ...
    def start_recording(self, measurements_manager: MeasurementsManager, name: str = Any, append_time: bool = True):
    ...
    def stop_recording(self) -> Optional[MeasurementsManager]:
        ...
        assert self._measurements_manager is not None
        return self._measurements_manager

    def _finish_and_get_samples(self):
        try:
            assert self._bg_client is not None
            self._bg_client.finish_measurements()
            samples = cast(SamplesList, self._bg_client.samples or self._bg_client.get_samples())
            assert self._measurements_manager is not None
